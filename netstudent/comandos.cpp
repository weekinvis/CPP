...
void cmdHelp() {
    cout << "\nAjuda:\n"
         << "'sair' -> Encerra a execucao do programa.\n"
         << "'ler' -> Le o arquivo atual e carrega os dados nas classes.\n"
         << "'sort [nome|nf|falta|aula]' -> Ordena os dados por criterio.\n"
         << "'media [nf|falta]' -> Exibe medias gerais.\n"
         ...
         << "'inserir' e 'terminar' -> Aciona um modo que permite a insercao pelo terminal.\n"
...
void cmdMedia(const string& arg) {
    if (materias.empty()) {
        cerr << "Erro: Nenhuma materia disponivel." << endl;
        return;
    }
    ...
    if (arg.find("nf") != string::npos) {
        ...
        cerr << "Erro: parametro invalido para media." << endl;
    }
}
...
bool processaComando(string& comandoCompleto) {
    ...
    cerr << "Comando nao encontrado." << endl;
    return false;
}
